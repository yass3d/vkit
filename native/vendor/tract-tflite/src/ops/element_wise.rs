...
fn de_prelu(op: &mut DeserOp) -> TractResult<TVec<OutletId>> {
    let dt = DatumType::super_type_for(op.facts()?.iter().map(|f| f.datum_type))
        .context("PReLU has no common datum type")?;
    let cast = wire_cast(format!("{}.cast", op.prefix), op.ctx.target, op.inputs, dt)?;
    let broadcast = wire_rank_broadcast(format!("{}.broadcast", op.prefix), op.ctx.target, &cast)?;
    let x = broadcast[0];
    let alpha = broadcast[1];
    let zero = op.ctx.target.add_const(format!("{}.zero", op.prefix), rctensor0(0.0f32))?;
    let xz = wire_rank_broadcast(format!("{}.zero_broadcast", op.prefix), op.ctx.target, &[x, zero])?;
    let positive = op.ctx.target.wire_node(
        format!("{}.positive", op.prefix),
        tract_core::ops::math::max(),
        &xz,
    )?[0];
    let negative = op.ctx.target.wire_node(
        format!("{}.negative", op.prefix),
        tract_core::ops::math::min(),
        &xz,
    )?[0];
    let scaled = op.ctx.target.wire_node(
