...
pub(super) const fn label(key: TextKey) -> Option<&'static str> {
    ...
    match key {
        ...
        TextKey::RecoveryLookMissing => {
            Some("O look daquela sessão não está mais instalado, então nada pôde ser restaurado.")
        }
        ...
        TextKey::SettingsBloomSoftKnee => Some("Transição suave"),
        TextKey::SettingsBloomRadius => Some("Raio"),
