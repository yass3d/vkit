...
pub(crate) fn draw_viewport_hair_panel_contents(ui: &mut Ui, state: &mut AppState) {
    set_capsule_widget_radius(ui);

    ui.label(
        RichText::new(text(state.locale, TextKey::HairPreviewCaveat))
            .size(FONT_XS)
            .color(COLOR_MUTED),
    );
    ui.add_space(SPACE_2);

    let mut visible = state.hair_visible;
    if hair_panel_switch_row(ui, &mut visible, text(state.locale, TextKey::HairVisible)) {
        state.dispatch(Action::ToggleHairVisible(visible));
    }
    ui.add_space(SPACE_2);
                ...
                ui.allocate_ui_with_layout(
                    ...
                    |ui| {
                        capsule_search_field(
                            ui,
                            "vkit.hair.search",
                            &mut state.hair_query,
                            text(state.locale, TextKey::HairSearch),
                            !state.vam_hair_presets.is_empty(),
                        );
                    },
                );
    ...
    ui.add_space(SPACE_2);
    let query = state.hair_query.trim().to_ascii_lowercase();
    let filtered = state
        .vam_hair_presets
        .iter()
        .enumerate()
        .filter_map(|(index, preset)| {
            ...
            let matches = query.is_empty()
                ...
                || preset.stable_id.to_ascii_lowercase().contains(&query);
            (compatible && matches).then_some(index)
        })
        .collect::<Vec<_>>();
    let mut selected = state.selected_hair_id.clone();
    let footer_reserve = f32::from(SKIN_SELECTOR_FRAME_MARGIN) * 2.0
        + panel_observed_tail(ui, HAIR_PANEL_TAIL)
            ...
            ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    if selectable_list_row(
                        ui,
                        text(state.locale, TextKey::HairNone),
                        selected.is_none(),
                    )
                    .clicked()
                    {
                    ...
                    for index in &filtered {
                        ...
                        let row_selected = selected.as_deref() == Some(preset.stable_id.as_str());
                        let available = !state.failed_hair_ids.contains(&preset.stable_id);
                        if selectable_list_row_styled(
                            ui,
    ...
    record_panel_tail(ui, HAIR_PANEL_TAIL, library_bottom);
    if selected != state.selected_hair_id {
        state.dispatch(Action::SelectVaMHair(selected));
    }
}

fn hair_panel_switch_row(ui: &mut Ui, on: &mut bool, label: &str) -> bool {
    ui.allocate_ui_with_layout(
        vec2(ui.available_width().max(0.0), CONTROL_HEIGHT),
        Layout::left_to_right(Align::Center),
        |ui| switch_row(ui, on, label).changed(),
    )
    .inner
}
...
pub(crate) const HAIR_PANEL_TAIL: &str = "hair";
